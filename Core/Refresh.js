...
function refresh(file) {
  ...
  if (!file) {
    CustomLogger.logError(`No file found in the + Monitoring Hourly folder.`, CONFIG.APP.NAME, "refresh()");
  ...
  try {
    ...
    CustomLogger.logError(e.message, CONFIG.APP.NAME, "refresh()");
    EmailSender.sendLogs(CONFIG.APP.ADMIN.email, CONFIG.APP.NAME);
    return;
  }
...
function setupFormulas(sheet, lastRow, sheetName) {
  ...
  CustomLogger.logInfo(`Applied all formulas for columns P-Y.`, CONFIG.APP.NAME, "setupFormulas()");
}
...
function populateNewSheet(sheetName = null, now = null) {
  ...
  if (existingSheet) {
    ...
    CustomLogger.logError(`Deleted existing sheet: ${sheetName}`, CONFIG.APP.NAME, "populateNewSheet()");
  }
  ...
  try {
    ...
    CustomLogger.logError(errorMsg, CONFIG.APP.NAME, "populateNewSheet()");
...
function sortLatestPercentage() {
  ...
  CustomLogger.logInfo(`Sorted latest percentage.`, CONFIG.APP.NAME, "sortLatestPercentage()");
}
...
function applyConditionalFormatting() {
  ...
  if (!sheet) {
    CustomLogger.logError("Sheet 'Kiosk %' not found.", CONFIG.APP.NAME, "applyConditionalFormatting()");
    return;
  }
  ...
  CustomLogger.logInfo(`Applied conditional formatting.`, CONFIG.APP.NAME, "applyConditionalFormatting()");
}
...
function deleteOldSheet() {
  ...
  if (startIndex >= sheets.length) {
    CustomLogger.logError(`Start index ${startIndex} is out of range.`, CONFIG.APP.NAME, "deleteOldSheet()");
    return;
  }
  ...
  for (let i = sheets.length - 1; i >= startIndex; i--) {
    ...
    CustomLogger.logInfo(`Deleted sheet: ${sheetName}`, CONFIG.APP.NAME, "deleteOldSheet()");
  }
}
