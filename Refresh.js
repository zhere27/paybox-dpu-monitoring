function refresh(file) {
  ...
  const formulaSparklineRange = sheet.getRange(`T2:T${lastRow}`)
    .clear()
    .setFontFamily('Century Gothic')
    .setFontSize(9)
    .setFormula(formulaSparkline);

  const formulaNoMovementRange = sheet.getRange(`U2:U${lastRow}`)
    .clear()
    .setFontFamily('Century Gothic')
    .setFontSize(9)
    .setFormula(formulaNoMovement);

  const formulaCollectedStoresRange = sheet.getRange(`V2:V${lastRow}`)
    .clear()
    .setFontFamily('Century Gothic')
    .setFontSize(9)
    .setFormula(formulaCollectedStores);

  const formulaFrequencyRange = sheet.getRange(`W2:W${lastRow}`)
    .clear()
    .setFontFamily('Century Gothic')
    .setFontSize(9)
    .setFormula(formulaFrequency);

  const formulaServicingBankRange = sheet.getRange(`X2:X${lastRow}`)
    .clear()
    .setFontFamily('Century Gothic')
    .setFontSize(9)
    .setFormula(formulaServicingBank);

  const formulaBusinessDaysRange = sheet.getRange(`Y2:Y${lastRow}`)
    .clear()
    .setFontFamily('Century Gothic')
    .setFontSize(9)
    .setFormula(formulaBusinessDays);
  ...
  const formulaRange = sheet.getRange(`P2:P${lastRow}`)
    .setFontFamily('Century Gothic')
    .setFontSize(9)
  ...
  const formulaCurrentAmountRange = sheet.getRange(`Q2:Q${lastRow}`)
    .clear()
    .setFontFamily('Century Gothic')
    .setFontSize(9)
...
function populateNewSheet(sheetName = null, now = null) {
  ...
  try {
    ...
    newSheet.showSheet();
    spreadSheet.moveActiveSheet(9); // Move the sheet to the 9th position
  } catch (e) {
