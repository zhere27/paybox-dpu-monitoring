function bpiBrinkCollectionsLogic() {
  CustomLogger.logInfo(
    "Running bpiBrinkCollectionsLogic...",
    PROJECT_NAME,
    "bpiBrinkCollectionsLogic()"
  );

  const environment = "production";
  const srvBank = "Brinks via BPI";
  var { todayDate, tomorrowDate, todayDay, tomorrowDateString } =
  ...
  machineNames.forEach((machineNameArr, i) => {
    ...
    const translatedBusinessDays = translateDaysToAbbreviation(
      businessDay.trim()
    );
  ...
  if (forCollections.length > 0) {
    ...
    CustomLogger.logInfo(
      "No eligible stores for collection tomorrow.",
      PROJECT_NAME,
      "bpiBrinkCollectionsLogic()"
    );
  }
