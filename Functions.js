...
/**
 *This will pull data from the database and populate the sheet
 *
 * @param {*} trnDate
 * @param {*} trnTime
 * @return {*} 
...
function getKioskData(srvBank) {
  ...
  const filteredData = collectionPartners.map((partner, index) => ({
    index,
    partner: partner[0]
  ...
  const filteredCollectionPartners = filteredData.map(i => collectionPartners[i]);
  const filteredCollectionSchedules = filteredData.map(i => collectionSchedules[i]);
  ...
  return [filteredMachineNames, filteredPercentValues, filteredAmountValues, filteredCollectionPartners, filteredCollectionSchedules, filteredPartnerAddress, filteredLastRequests, filteredBusinessDays];
}
